...
function test_resiliency_failure_with_server_down_and_single_disk_offline() {
	...
	docker exec resiliency-minio2-1 /bin/bash -c "mv /data2/.minio.sys /data2/.minio.bkp && touch /data2/.minio.sys"
	docker exec resiliency-minio3-1 /bin/bash -c "mv /data6/.minio.sys /data6/.minio.bkp && touch /data6/.minio.sys"
	sleep 10

	verify_resiliency_failure "${FUNCNAME[0]}"

	# Enable the disks back on nodes
	docker exec resiliency-minio2-1 /bin/bash -c "rm -rf /data2/.minio.sys && mv /data2/.minio.bkp /data2/.minio.sys"
	docker exec resiliency-minio3-1 /bin/bash -c "rm -rf /data6/.minio.sys && mv /data6/.minio.bkp /data6/.minio.sys"
...
function test_resiliency_success_with_disks_offline() {
	...
	docker exec resiliency-minio1-1 /bin/bash -c "mv /data1/.minio.sys /data1/.minio.bkp && touch /data1/.minio.sys"
	docker exec resiliency-minio1-1 /bin/bash -c "mv /data5/.minio.sys /data5/.minio.bkp && touch /data5/.minio.sys"
	docker exec resiliency-minio2-1 /bin/bash -c "mv /data1/.minio.sys /data1/.minio.bkp && touch /data1/.minio.sys"
	docker exec resiliency-minio2-1 /bin/bash -c "mv /data5/.minio.sys /data5/.minio.bkp && touch /data5/.minio.sys"
	docker exec resiliency-minio3-1 /bin/bash -c "mv /data1/.minio.sys /data1/.minio.bkp && touch /data1/.minio.sys"
	docker exec resiliency-minio3-1 /bin/bash -c "mv /data5/.minio.sys /data5/.minio.bkp && touch /data5/.minio.sys"
	docker exec resiliency-minio4-1 /bin/bash -c "mv /data1/.minio.sys /data1/.minio.bkp && touch /data1/.minio.sys"
	docker exec resiliency-minio4-1 /bin/bash -c "mv /data5/.minio.sys /data5/.minio.bkp && touch /data5/.minio.sys"
	sleep 10

	verify_resiliency "${FUNCNAME[0]}"

	# Finally enable the disks back on nodes
	docker exec resiliency-minio1-1 /bin/bash -c "rm -rf /data1/.minio.sys && mv /data1/.minio.bkp /data1/.minio.sys"
	docker exec resiliency-minio1-1 /bin/bash -c "rm -rf /data5/.minio.sys && mv /data5/.minio.bkp /data5/.minio.sys"
	docker exec resiliency-minio2-1 /bin/bash -c "rm -rf /data1/.minio.sys && mv /data1/.minio.bkp /data1/.minio.sys"
	docker exec resiliency-minio2-1 /bin/bash -c "rm -rf /data5/.minio.sys && mv /data5/.minio.bkp /data5/.minio.sys"
	docker exec resiliency-minio3-1 /bin/bash -c "rm -rf /data1/.minio.sys && mv /data1/.minio.bkp /data1/.minio.sys"
	docker exec resiliency-minio3-1 /bin/bash -c "rm -rf /data5/.minio.sys && mv /data5/.minio.bkp /data5/.minio.sys"
	docker exec resiliency-minio4-1 /bin/bash -c "rm -rf /data1/.minio.sys && mv /data1/.minio.bkp /data1/.minio.sys"
	docker exec resiliency-minio4-1 /bin/bash -c "rm -rf /data5/.minio.sys && mv /data5/.minio.bkp /data5/.minio.sys"
...
function find_erasure_set_for_file() {
	...
	docker exec resiliency-minio1-1 /bin/bash -c "stat /data1/test-bucket/$DIR/$FILE/xl.meta" >/dev/null 2>&1
	STATUS=$?
	if [ $STATUS -eq 0 ]; then
		DATA_DRIVE=1
	fi
	...
	if [ $DATA_DRIVE -eq -1 ]; then
		...
		docker exec resiliency-minio1-1 /bin/bash -c "stat /data5/test-bucket/$DIR/$FILE/xl.meta" >/dev/null 2>&1
		STATUS=$?
		if [ $STATUS -eq 0 ]; then
			DATA_DRIVE=5
		fi
...
function test_resiliency_healing_missing_xl_metas() {
	...
	OUTPUT=$(docker exec resiliency-minio1-1 /bin/bash -c "rm /data$((DATA_DRIVE))/test-bucket/initial-data/$FILE/xl.meta")
	...
	OUTPUT=$(docker exec resiliency-minio1-1 /bin/bash -c "rm /data$((DATA_DRIVE))/test-bucket/initial-data/$FILE/xl.meta")
	OUTPUT=$(docker exec resiliency-minio2-1 /bin/bash -c "rm /data$((DATA_DRIVE + 1))/test-bucket/initial-data/$FILE/xl.meta")
	...
	OUTPUT=$(docker exec resiliency-minio1-1 /bin/bash -c "rm /data$((DATA_DRIVE))/test-bucket/initial-data/$FILE/xl.meta")
	OUTPUT=$(docker exec resiliency-minio2-1 /bin/bash -c "rm /data$((DATA_DRIVE + 1))/test-bucket/initial-data/$FILE/xl.meta")
	OUTPUT=$(docker exec resiliency-minio3-1 /bin/bash -c "rm /data$((DATA_DRIVE + 2))/test-bucket/initial-data/$FILE/xl.meta")
	...
	OUTPUT=$(docker exec resiliency-minio2-1 /bin/bash -c "rm /data$((DATA_DRIVE + 1))/test-bucket/initial-data/$FILE/xl.meta")
	OUTPUT=$(docker exec resiliency-minio3-1 /bin/bash -c "rm /data$((DATA_DRIVE + 2))/test-bucket/initial-data/$FILE/xl.meta")
	OUTPUT=$(docker exec resiliency-minio4-1 /bin/bash -c "rm /data$((DATA_DRIVE + 3))/test-bucket/initial-data/$FILE/xl.meta")
...
function test_resiliency_healing_truncated_parts() {
	...
	OUTPUT=$(docker exec resiliency-minio1-1 /bin/bash -c "truncate --size=10K /data$((DATA_DRIVE))/test-bucket/initial-data/$FILE/*/part.1")
	...
	OUTPUT=$(docker exec resiliency-minio2-1 /bin/bash -c "truncate --size=10K /data{$((DATA_DRIVE))..$((DATA_DRIVE + 1))}/test-bucket/initial-data/$FILE/*/part.1")
	...
	OUTPUT=$(docker exec resiliency-minio3-1 /bin/bash -c "truncate --size=10K /data{$((DATA_DRIVE))..$((DATA_DRIVE + 2))}/test-bucket/initial-data/$FILE/*/part.1")
	...
	OUTPUT=$(docker exec resiliency-minio4-1 /bin/bash -c "truncate --size=10K /data{$((DATA_DRIVE))..$((DATA_DRIVE + 3))}/test-bucket/initial-data/$FILE/*/part.1")
...
function induce_bitrot() {
	...
	UUID=$(docker exec resiliency-minio$NODE-1 /bin/bash -c "ls -l $DIR/test-bucket/initial-data/$FILE/*/part.1")
	UUID=$(echo $UUID | cut -d " " -f 9 | cut -d "/" -f 6)

	# Determine head and tail size of file where we will introduce bitrot
	FILE_SIZE=$(docker exec resiliency-minio$NODE-1 /bin/bash -c "stat --printf="%s" $DIR/test-bucket/initial-data/$FILE/$UUID/part.1")
	TAIL_SIZE=$((FILE_SIZE - 32 * 2))

	# Extract head and tail of file
	$(docker exec resiliency-minio$NODE-1 /bin/bash -c "cat $DIR/test-bucket/initial-data/$FILE/$UUID/part.1 | head --bytes 32 > /tmp/head")
	$(docker exec resiliency-minio$NODE-1 /bin/bash -c "cat $DIR/test-bucket/initial-data/$FILE/$UUID/part.1 | tail --bytes $TAIL_SIZE > /tmp/tail")

	# Corrupt the part by writing head twice followed by tail
	$(docker exec resiliency-minio$NODE-1 /bin/bash -c "cat /tmp/head /tmp/head /tmp/tail > $DIR/test-bucket/initial-data/$FILE/$UUID/part.1")
}
...
function induce_bitrot_for_xlmeta() {
	...
	# Determine head and tail size of file where we will introduce bitrot
	FILE_SIZE=$(docker exec resiliency-minio$NODE-1 /bin/bash -c "stat --printf="%s" $DIR/test-bucket/inlined-data/$FILE/xl.meta")
	HEAD_SIZE=$((FILE_SIZE - 32 * 2))

	# Extract head and tail of file
	$(docker exec resiliency-minio$NODE-1 /bin/bash -c "cat $DIR/test-bucket/inlined-data/$FILE/xl.meta | head --bytes $HEAD_SIZE > /head")
	$(docker exec resiliency-minio$NODE-1 /bin/bash -c "cat $DIR/test-bucket/inlined-data/$FILE/xl.meta | tail --bytes 32 > /tail")

	# Corrupt xl.meta by writing head followed by tail twice
	$(docker exec resiliency-minio$NODE-1 /bin/bash -c "cat /head /tail tmp/tail > $DIR/test-bucket/inlined-data/$FILE/xl.meta")
}
